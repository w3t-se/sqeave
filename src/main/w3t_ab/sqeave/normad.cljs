...
#_(defn add [{:keys [store setStore] :as ctx} & data]
  (let [res (batch #(traverse-and-transform (or (first data) store) acc))]
    ...
    (if-not (first data)
      ...
      (reduce-kv (fn [m k v] (log/debug "set: " k " " v) #_(log/debug "set2: " (merge-with merge (get store k) v #_{:key k :merge true})) (setStore k #(merge-with merge % v #_{:key k :merge true})))
                 {}
                 (if-not (vector? res)
                   (merge  @acc res)
                   @acc))
